...
fn insert(connection: &PgConnection, user: &str, name: &str, code: &str) -> Result<()> {
    ...
    Version::parse(&version).context("Version is invalid")?;
    ...
    connection.transaction::<_, Error, _>(|| {
        ...
        match Release::try_find(module.id, &version, &connection)? {
            ...
            None => module
                .add_version(&version, &code, &connection)
...
fn run() -> Result<()> {
    ...
    insert(
        ...
        "kpcyrd",
        "ctlogs",
        r#"-- Description: Query certificate transparency logs to discover subdomains
-- Version: 0.5.0
-- Source: domains
-- License: GPL-3.0
...
"#,
    ...
    insert(
        ...
        "dead",
        "module",
        r#"-- Description: TODO your description here
-- Version: 0.1.0
-- License: GPL-3.0
